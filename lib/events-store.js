...
function wrapDatabaseError(error) {
  ...
  if (error?.code === "42703") {
    throw createHttpError(
      500,
      "La base no tiene las columnas de colores. Ejecuta `npm run db:init` y vuelve a intentar."
    );
  }
...
async function listEvents() {
  try {
    ...
    const rows = await sql`
      SELECT id, title, theme, palette, background_color, progress_color, text_color, date, created_at
      FROM events
...
async function createEvent(payload) {
  try {
    ...
    const rows = await sql`
      INSERT INTO events (id, title, theme, palette, background_color, progress_color, text_color, date)
      VALUES (
        ${input.id},
        ${input.title},
        ${input.theme},
      ...
      RETURNING id, title, theme, palette, background_color, progress_color, text_color, date, created_at
    `;
...
async function updateEvent(id, payload) {
  ...
  try {
    ...
    if (input.title !== undefined) updates.push(sql`title = ${input.title}`);
    if (input.theme !== undefined) updates.push(sql`theme = ${input.theme}`);
    if (input.palette !== undefined) updates.push(sql`palette = ${input.palette}`);
    if (input.colors !== undefined) {
      updates.push(sql`background_color = ${input.colors.background}`);
      updates.push(sql`progress_color = ${input.colors.progress}`);
      updates.push(sql`text_color = ${input.colors.text}`);
    }
    if (input.date !== undefined) updates.push(sql`date = ${input.date}`);
